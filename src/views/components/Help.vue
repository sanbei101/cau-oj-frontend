<template>
  <n-config-provider abstract :hljs="highlightJs">
    <div class="wrap help">
      <n-h2>语言支持</n-h2>
      <n-data-table single-column :columns="languageColumns" :data="languageData" />
      <n-h2>注意事项</n-h2>
      <n-alert :bordered="false" type="info" title="Java"> Java 的类名必须为 Solution,且不能使用 package 关键字</n-alert>
      <n-alert :bordered="false" type="info" title="Kotlin Native" style="margin-top: 24px"> Kotlin 编译时间可能长达 30 秒以上</n-alert>
      <n-alert :bordered="false" type="info" title="输入输出" style="margin-top: 24px">
        <ul>
      ...
      <n-alert type="info" :bordered="false" style="margin-bottom: 24px">
        若提交成功一直获取不到结果(下表前 3 种状态),可以稍后查询提交记录,若一直没有结果,请联系管理员。
      </n-alert>
      <n-data-table single-column :columns="judgeColumns" :data="judgeData" />
      <n-alert :bordered="false" type="info" title="部分状态与错误说明" style="margin-top: 24px">
        <ul>
...
import { NAlert, NCard, NCode, NConfigProvider, NDataTable, NH2, NH3 } from 'naive-ui';
