<template>
  <n-space vertical align="center" size="large" item-style="width: 100%">
    <!-- 错误 -->
    <n-result v-if="error != null" size="small" status="warning" :title="error.error" :description="error.message">
    </n-result>
    <!-- 结果 -->
    <n-result v-else size="small" :status="result.status">
      <n-table size="small" style="text-align: center; margin-bottom: 12px" :single-line="false">
        <thead>
          <tr>
            <th>状态</th>
            <th>CPU 时间</th>
...
function fetchResult() {
  sse = new EventSource(`${ApiPath.SOLUTION}/time/${props.submitTime}?token=${store.user.userInfo?.token}`);
