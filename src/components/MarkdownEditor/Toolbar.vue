<template>
  <n-button-group size="small">
    ...
    <n-dropdown placement="bottom-start" size="small" :show-arrow="true" :options="imgOptions" @select="click">
      <n-button tertiary>
        <n-icon :size="iconSize">
          <image-icon />
        </n-icon>
...
import { NSpace, NButtonGroup, NButton, NInputNumber, NDropdown, NIcon } from 'naive-ui';
import {
...
const tableOptions = [
  {
    ...
    render: () => (
      <NSpace vertical={true} style="padding: 0 6px">
        ...
        <NButton type="primary" onClick={() => insertTable()} style="width: 100%">
          插入表格
        </NButton>
      </NSpace>
    )
