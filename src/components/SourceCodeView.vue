<template>
  <n-config-provider abstract :hljs="highlightJs">
    <n-page-header @back="back">
      ...
      <template #extra>
        <router-link :to="{ name: 'submission', params: { pid: data.problemId } }">
          <n-button quaternary type="info">进入题目页面</n-button>
        </router-link>
      </template>
      <n-space vertical>
        <n-table v-if="showUser" size="small" style="text-align: center" :single-line="false">
          <thead>
            <tr>
              <th>Solution ID</th>
              <th>UID</th>
        ...
        <n-table size="small" style="text-align: center" :single-line="false">
          ...
          <tbody>
            <tr>
              ...
              <td>
                <n-text strong :style="{ color: LanguageColors[data.language!] }">
        ...
        <n-card>
          <n-code trim show-line-numbers :code="data.sourceCode" :language="langs[data.language!]" />
        </n-card>
      </n-space>
      <template v-if="data.errorInfo" #footer>
        <n-divider dashed title-placement="left" style="margin-top: 0; margin-bottom: 14px"> 错误信息 </n-divider>
        <n-card>
          <n-text type="error">
...
import { NButton, NCard, NCode, NConfigProvider, NDivider, NPageHeader, NSpace, NTable, NText } from 'naive-ui';
